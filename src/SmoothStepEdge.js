  ...
  ({
    ...
    return (
      ...
      <defs>
        ...
        <path style={{stroke: isEdgeHovered ? 'red' : "#3C639B", cursor: 'pointer', pointerEvents: 'auto', strokeWidth: isEdgeHovered ? '3px' : '1.5px'}} className="react-flow__edge-path" d={path} markerEnd={`url(#check1)`} onMouseEnter={()=>{setEdgeHovered(true)}} onMouseLeave={()=>setEdgeHovered(false)}/>
        {isEdgeHovered && text}
      </>
    );
  }
